...
func (g Galleries) UploadImage(w http.ResponseWriter, r *http.Request) {
	...
	err = r.ParseMultipartForm(5 << 20) // 5mb bit shift
	if err != nil {
		http.Error(w, "Something went wrong", http.StatusInternalServerError)
		return
	}
	...
	for _, fileHeader := range fileHeaders {
		...
		if err != nil {
			...
			if errors.As(err, &fileErr) {
				msg := fmt.Sprintf("%v has an invalid content type or extension. Only png, gif and jpeg are supported.", fileHeader.Filename)
...
func (g Galleries) ImageViaURL(w http.ResponseWriter, r *http.Request) {
	...
	for _, file := range files {
		err = g.ImageService.CreateImageViaURL(gallery.ID, file)
		if err != nil {
